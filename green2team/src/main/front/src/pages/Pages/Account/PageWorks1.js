...
function PageWorks1() {
    ...
    const [panoramaViewerUrl, setPanoramaViewerUrl] = useState("");
    ...
    // 내용과 URL 분리
    const extractContentAndPanorama = (data) => {
        const parser = new DOMParser();
        const parsedHtml = parser.parseFromString(data, 'text/html');
        const oembedElement = parsedHtml.querySelector('figure.media oembed');
        let content = data;
        let panoramaViewerUrl = '';
    
        if (oembedElement) {
            panoramaViewerUrl = `<figure class="media">${oembedElement.outerHTML}</figure>`;
        ...
        return { content, panoramaViewerUrl };
    };
    ...
    const handleSubmit = (e) => {
        ...
        const { content, panoramaViewerUrl } = extractContentAndPanorama(editorData);
        formData.append("title", title);
        formData.append("contents", content);
        formData.append("url", panoramaViewerUrl);
        ...
        axios.put(`/api/modifyVrBoard/${vrBoardNo}`, formData)
            .then(response => {
                alert("수정되었습니다.");
                navigate("/page-invoice");
            });
            // .catch(error => {
    ...
    useEffect(() => {
        axios.get(`/page-case-detail-modify/${vrBoardNo}`)
          .then(response => {
            const { vrBoardTitle, vrBoardContent, vrThumbnailImgUrl , vrPanoramaViewerUrl } = response.data;
            setTitle(vrBoardTitle);
            setContent(vrBoardContent);
            setPanoramaViewerUrl(vrPanoramaViewerUrl);
            setThumbnailUrl(vrThumbnailImgUrl);

            // CKEditor 초기화
            ClassicEditor
            .create(document.querySelector('#editor'), {height: '300px', shouldNotToolbar: true})
            .then(editor => {
                editor.setData(vrPanoramaViewerUrl + vrBoardContent);
       ...
       <div className="container-fluid">
            <div className="layout-specing">
                <div className="d-md-flex justify-content-between align-items-center">
                    <h5 className="mb-0">아카이브 콘텐츠 수정</h5>
                    ...
                    <nav aria-label="breadcrumb" className="d-inline-block mt-2 mt-sm-0">
                        <ul className="breadcrumb bg-transparent rounded mb-0 p-0">
                            ...
                            <li className="breadcrumb-item text-capitalize"><a href="/page-invoice">아카이브 콘텐츠</a></li>
